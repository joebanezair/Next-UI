...
const AccountFormSignUp = () => {
  ...
  const conf_pass = jsontext.English.globalTextEnglish.ConfirmPassword; 
  const enter_name = "Given Name";
  const enter_lastname = "Last Name";
    ...
    <div style={{maxWidth: 500}} className="mx-auto">
      <Card className="py-4 rounded-md w-126 max-w-126">
        <CardHeader className="pb-0 pt-2 px-4 flex-col items-start">
          <h3 className="text-large font-bold">{jsontext.English.globalTextEnglish.OffSign}</h3>
        </CardHeader>
        <CardBody className="pb-0 pt-2 px-4 flex-col items-start overflow-visible py-0">
         <div className="flex w-full mt-3 mb-0 flex-wrap md:flex-nowrap gap-2">
          <Input type="text" 
            value={email}
          ...
          <Input type="text" 
            value={email}
            onChange={(e)=>setEmail(e.target.value)}
            label={enter_lastname} />
         </div>
         <div className="flex w-full mt-2 mb-2 flex-wrap md:flex-nowrap gap-2">
          <Input type="email" 
            value={email}
            onChange={(e)=>setEmail(e.target.value)}
            label={enter_email} />
         </div>
         <div className="flex w-full mt-0 mb-3 flex-wrap md:flex-nowrap gap-2">
          <Input type="password"
            value={password}
...
export default AccountFormSignUp